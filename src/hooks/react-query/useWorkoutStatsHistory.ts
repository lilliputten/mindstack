...
export function useWorkoutStatsHistory(topicId?: TTopicId) {
  ...
  const queryKey = React.useMemo(
    () => ['workout-stats-history', topicId, userId],
    [topicId, userId],
  );
  ...
  const queryFn = React.useCallback(async () => {
    ...
    try {
      workoutStats = await Promise.race([
        getWorkoutStatsHistory(topicId),
      ...
      if (!memo.mounted) {
        ...
        console.warn('[useWorkoutStatsHistory:queryFn]', message, { topicId });
    ...
    const accuracyStdDev = Math.sqrt(
      accuracies.reduce((sum, acc) => sum + Math.pow(acc - averageAccuracy, 2), 0) / totalWorkouts,
    );
